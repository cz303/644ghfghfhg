...
const Администраторs = [283698059]; //// id админа (для логов)
const vip = [283698059]; /////// id админа (для логов)
...
let chatid = message.chatId;	
		...
		message.send(`😜 || Приветствую тебя, игрок - ${user[0].first_name}! Это крутой игровой Banana Bot .\n\n😃 >> Чтобы начать играть, нажмите нужную кнопку, ну или написать команду - Помощь`, 
		{
...
cmd.hear(/^(?:товары|донат|привилегии|tovar|tovars)$/i, (message) => {  
	return message.send(
		`🔸 Донат 🔸
✅ || Привилегия: Vip 
🍏 | Стоимость: 149 RUB 
...
[💬] Команды: 
1) /givemoney - Выдать деньги у бота
2) /ban - Заблокировать пользователя 
3) /unban - Разблокировать пользователя 
4) /kick - Исключить пользователя из беседы 
...
2) /warn - Выдать выговор пользователю рангом ниже вашего 
3) /unwarn - Снять выговор пользователю 
4) /givemoney - Выдать деньги у бота 
5) /givebtc - Выдать btc у бота
и другие.

Так же доступны команды , Vip , Администратор 

➖➖➖➖➖➖➖➖➖➖➖➖➖➖➖➖➖

🙈 >> Все товары приобретаются на сайте - bananabot.tk, либо командой - Купить (Номер товара).
🤠 >> Связаться с Администрацией, командой - репорт - @bot_banana (Banana Bot)!
💸 >> На вашем балансе: ${message.user.reals}₽`
	);	
});
cmd.hear(/^(?:Купить 1)$/i, async (message, bot) => {
	message.user.foolder += 1;
	...
	if(message.user.reals < 149) return bot(`пополните баланс! Команда - товары`);
{
...
cmd.hear(/^(?:Купить 2)$/i, async (message, bot) => {
	message.user.foolder += 1;
    
	if(message.user.right == 3) return bot(`вы уже имеете данную привилегию...`);
	if(message.user.right == 4) return bot(`вы уже имеете более высокую привилегию...`);
	if(message.user.reals < 1000) return bot(`пополните баланс! Команда - товары`);
{
...
cmd.hear(/^(?:Купить 3)$/i, async (message, bot) => {
	message.user.foolder += 1;

	if(message.user.right == 4) return bot(`вы уже имеете данную привилегию...`);
	if(message.user.reals < 2500) return bot(`пополните баланс! Команда - товары`);
{
message.user.reals -= 2500,
message.user.right = 4;
return message.send(`👍🏻 || Вы успешно купили привилегию: Гл.Администратора`);
}
});
...
return bot(`введите номер товара. Пример - Купить 1 (Вип)!`);
});
...
cmd.hear(/^(?:найти)\s([0-9]+)$/i, async (message, bot) => {
	...
	text += `\n🔥 ${user.right.toString().replace(/1/gi, "Привилегия: Пользователь").replace(/2/gi, "Привилегия: Вип").replace(/3/gi, "Привилегия: Администратор").replace(/4/gi, "Привилегия: Гл.Администратор")}`;
...
cmd.hear(/^(?:Я|Профиль)$/i, async (message, bot) => {
	message.user.foolder += 1;
			let text = ``;	
    ...
    text += `&#4448;🔥 ⇢ Привилегия: ${message.user.right.toString().replace(/1/gi, "Пользователь").replace(/2/gi, "Вип").replace(/3/gi, "Администратор").replace(/4/gi, "Гл.Администратор").replace(/5/gi, "[Samids|Владелец]")}`;
...
if(zapret.test(zaprets1) == true){
    var check = true;
    return message.reply(`[🆘] » Ой... Похоже, что [bot_banana|Banana Bot] заблокировал запрещенные слова/символы/смайлики.\n\n[😉] » Чтобы убрать блокировку, Вы можете приобрести донат Vip и выше в группе. || [bot_banana|Banana Bot]`);
}
...
cmd.hear(/^(?:репорт|реп|rep|жалоба)\s([^]+)$/i, async (message, bot) => {
	if(message.isChat) return bot(`😉 По всем вопросам - [283698059|сюда❗]`);
	...
	vk.api.messages.send({ user_id: 283698059, forward_messages: message.id, message: `Player id: ${message.user.uid}` }).then(() => {
		...
		return bot(`По всем вопросам![283698059|Администратор]`);
	});
});

cmd.hear(/^(?:ответ)\s([0-9]+)\s([^]+)$/i, async (message, bot) => {
	message.user.foolder += 1;
	if(message.user.right < 3) return bot(`доступно с привилегии Администратором.`);
		if(message.args[2] <= 0) return;
...
[Тех Поддержка] - ${new Date().getHours()}:${new Date().getMinutes()}
...
${message.user.misc.farm === 6 ? '🔋' : '🔋'}6. BANAN H10 150.000Ƀ/час (1.000.000.000.000.000$)
...
cmd.hear(/^(?:очистить чат|clear)$/i, async (message, bot) => {
	message.user.foolder += 1;
...
cmd.hear(/^(?:!Администратор|!админ|!администратор)\s(.*)$/i, async (message, bot) => {
	message.user.foolder += 1;
let chatid = message.chatId;	
	if(chats[message.chatId].users[message.user.id].group < 3) return;
...
cmd.hear(/^(?:!permban)\s(.*)$/i, async (message, bot) => {
	...
	if(chats[message.chatId].users[message.user.id].group < 4) return;
		let argses = message.text.split("!permban ");
			...
			argses[1] = argses[1].replace(/\//ig, '');
			let user = await vk.api.utils.resolveScreenName({screen_name: argses[1]});
			if(user.object_id == 283698059) return message.reply('пошол нах');
			if(!chats[message.chatId].users[user.object_id]) return message.send(`❌ Участник не найден!`);
...
/* |                                | */
/* | А тут уже идут Администратор команды | */
/* V                                V */
...
message.user.foolder += 1;
  ...
  vk.api.messages.send({ user_id: zz.id, message: `${message.args[1]}`}); 
        ...
        return message.send(`💬 || Я успешно сделал рассылку! Посмотрите, как будет видно другим:\n\n[bot_banana|Banana Bot], Сегодня в ${new Date().getHours()}:${new Date().getMinutes()}:${new Date().getSeconds()}\n"${message.args[1]}"`);
...
[💣] || Вас забанил: + АдминистраторISTRATOR + 
[⚠] || Вы забанены с ${new Date().getHours()}:${new Date().getMinutes()} - НАВЕЧНО

🍀 Вы можете купить разбан в группе - [bot_banana|Banana Bot]
🎁 Летние скидки %` }); 
...
cmd.hear(/^(?:Беседы)$/i, async (message, bot) => {
	message.user.foolder += 1;
			...
			if(message.user.keyboard == false) {
					await message.reply(`[👑] || Беседы:

1. [Banana Bot | Чат бота] - https://vk.me/join/AJQ1dydF0hBgusYkAAOOS45R
2. [Ледяное Общество] - Не указан
`);
}
if(message.user.keyboard == true) {
	await message.reply(`[👑] || Беседы:
...
cmd.hear(/^(?:Otvet)\s([0-9]+)\s([^]+)$/i, async (message, bot) => {
	let user = users.users[user_id(message.user)];
		if(!Number(message.$match[1]) || !message.$match[1] || !message.$match[2] || !users.users[message.$match[1]]) return message.send(`Проверьте вводимые данные.`);
		let a = zapret(message.$match[2]);
		if(a != 0) return message.send(a); 
		vk.api.call("messages.send", {
			peer_id: users.users[message.$match[1]].id,
			message: `@bot_banana (TREST.)  — Админ: ${user.prefix} ответил Вам: \n${message.$match[2]}`
	...
	cmd.hear(/^(?:log)\s?([0-9]+)?\s?([0-9]+)?/i, (message) => {
		...
		if(!message.$match[2]) return message.send(`- - log [id] [number] - -\n1. Передачи [передать]\n2. Выдачи [give]\n3. Обнуления [remove]\n4. Выдача прав [Администратор]\n5. Обнуление прав [Администратор]\n6. Варны [warn]`) 
		...
		if(i == 4) for(i=0; i!=log.Администратор[id].log.length; i++){text += log.Администратор[id].log[i];} 
...
[❤] || Вас разабанил: + АдминистраторISTRATOR + ` }); 
